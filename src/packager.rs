...
pub fn pack<const MAX_DATA: usize, const MAX_PACKET: usize>(
    ...
    if direction == Direction::Request {
        let encrypted_key = package.private_key ^ package.private_key;
        raw_data.push(encrypted_key).map_err(|_| PackError::BufferTooSmall)?;
    }

    for &byte in package.data.iter() {
        let b = if package.protected { byte ^ package.private_key } else { byte };
...
mod tests {
    ...
    fn test_protected_response_roundtrip() {
        ...
        let packed: HVec<u8, 16> = pack(&package, Direction::Response)
            .expect("Pack failed");
        ...
        assert_eq!(unpacked.private_key, 0xBA);
    ...
    fn test_pack_buffer_too_small() {
        ...
        let result: Result<HVec<u8, 5>, PackError> = pack(&package, Direction::Request);
    ...
    fn test_protected_pack() {
        ...
        let packed_data: heapless::Vec<u8, 32> = pack(&package, Direction::Response).unwrap();
        let expected_packet = 
